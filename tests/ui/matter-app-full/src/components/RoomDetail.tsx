...
export function RoomDetail() {
  ...
  const { selectedWasters, setSelectedWasters, houseData, handleResetDemo } = useOutletContext<ContextType>();
  ...
  const toggleAllInCategory = (category: string, wasters: EnergyWaster[]) => {
    ...
    setSelectedWasters(prev => {
      ...
      if (allSelected) {
        ...
        wasters.forEach(w => newSet.delete(w.id));
      } else {
        // Select all in this category
        wasters.forEach(w => newSet.add(w.id));
      }
      return newSet;
    });
  };

  // Group energy wasters by type
  const groupedWasters = room.energyWasters.reduce((acc, waster) => {
    if (!acc[waster.type]) {
      acc[waster.type] = [];
    }
    acc[waster.type].push(waster);
    return acc;
  }, {} as Record<string, EnergyWaster[]>);
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 md:px-8 md:py-4 border-b border-gray-500/50 flex-shrink-0 backdrop-blur-sm">
        <div className="flex items-center gap-2 md:gap-4">
          <button
            onClick={handleBack}
            className="flex items-center gap-1 md:gap-2 p-2 md:px-4 md:py-2.5 rounded-xl hover:bg-gray-600/50 text-white transition-all backdrop-blur-sm border border-gray-500/30 hover:border-gray-400/50"
          >
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-2 md:p-2.5 hover:bg-gray-600/50 rounded-xl disabled:opacity-30 disabled:cursor-not-allowed transition-all border border-gray-400/30 hover:border-gray-300/50 backdrop-blur-sm"
            ...
            <div className="flex flex-col md:flex-row gap-2 md:gap-8 flex-1 overflow-hidden">
                    ...
                    return (
                      ...
                      >
                        ...
                        <rect
                          ...
                          height="70"
                          fill="white"
                          stroke={isSelected ? "#1f2937" : waster.isProblem ? "#ef4444" : "#9ca3af"}
                        ...
                        <g transform={`translate(${x}, ${y})`}>
                          <Icon
                            ...
                            width={32}
                            height={32}
                            className={waster.isProblem ? "text-red-600" : "text-gray-700"}
                          />
                        </g>
              ...
              >
                <div className="flex flex-row md:flex-row h-full">
                  {/* Main Header - Just chevron */}
                  <button
                    onClick={() => setIsDeviceListExpanded(!isDeviceListExpanded)}
                    className="flex-shrink-0 w-12 flex items-center justify-center p-2 md:p-3 hover:bg-gray-600/50 transition-all border-r border-gray-500/40 h-full md:h-auto backdrop-blur-sm"
                  >
                    <motion.div
                      animate={{ rotate: isDeviceListExpanded ? -90 : 90 }}
                      ...
                      >
                        <div className="p-2 space-y-1.5 md:space-y-2 w-64">
                          {Object.entries(groupedWasters).map(([category, wasters], categoryIndex) => {
                            ...
                            const categorySelectedCount = wasters.filter(w => selectedWasters.has(w.id)).length;
                            const allCategorySelected = wasters.every(w => selectedWasters.has(w.id));

                            return (
                              <div key={category} className="border border-gray-500/40 rounded-xl overflow-hidden bg-gradient-to-br from-[#374151] to-[#2d3748] shadow-lg">
                                {/* Category Header */}
                                <motion.div
                                ...
                                >
                                  ...
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      toggleAllInCategory(category, wasters);
                                    }}
                                    className="flex-shrink-0 p-2 md:p-3 hover:bg-gray-700 transition-colors"
                                  >
                                    <div
                                      className={`w-3.5 h-3.5 md:w-4 md:h-4 rounded-full border-2 flex items-center justify-center transition-all ${
                                        allCategorySelected
                                  ...
                                  <button
                                    ...
                                    className="flex-1 flex items-center justify-between p-2 md:p-3 hover:bg-gray-700 transition-colors"
                                  >
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-xs text-gray-400">
                                        ({categorySelectedCount}/{wasters.length})
                                      </span>
                                    </div>
                                    <motion.div
                                  ...
                                  {isExpanded && (
                                    ...
                                    >
                                      <div className="p-1 md:p-2 space-y-1">
                                        {wasters.map((waster: EnergyWaster, index) => {
                                          const isSelected = selectedWasters.has(waster.id);
                                          ...
                                          return (
                                            <motion.button
                                              ...
                                              onMouseEnter={() => setHoveredWaster(waster.id)}
                                              onMouseLeave={() => setHoveredWaster(null)}
                                              className={`w-full text-left p-2.5 rounded-xl transition-all backdrop-blur-sm ${
                                                isSelected
                                                  ? 'bg-gray-700/60 shadow-inner'
                                                  : waster.isProblem
                                                  ? 'hover:bg-gray-600/40'
                                                  : 'hover:bg-gray-600/40'
                                              } ${waster.isProblem ? 'border-l-3 border-red-500' : ''}`}
                                            >
                                              <div className="flex items-center gap-2">
                                                {/* Selection Indicator */}
                                                <div
